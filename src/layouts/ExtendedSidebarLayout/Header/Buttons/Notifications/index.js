...
function HeaderNotifications() {
  ...
  let stockNotes = useSelector((state) => state.notification.items);
  stockNotes = stockNotes.filter((i) => !i.isRead);
  const stockCount = stockNotes.length;
  useEffect(() => {
  ...
  const goAll = () => {
    setOpen(false);
    return navigate('/notifications/list');
  };
    ...
    <>
      <Tooltip arrow title={t('Notifications')}>
        <Badge
          ...
          sx={{
            '.MuiBadge-badge': {
              background: stockCount > 0 ? theme.colors.error.main : '',
